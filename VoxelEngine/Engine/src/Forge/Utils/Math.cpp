...
namespace Math
{
    void PerlinNoise::GenerateRandomValues()
    {
        RandomArray = {
        ...
        RandomArray.insert(RandomArray.end(), RandomArray.begin(), RandomArray.end());
    }
    //sinstance should be ouside?????????????????????????
    void PerlinNoise::GenerateRandomValues(uint32_t seed)
    {
        RandomArray.resize(256);
        std::iota(RandomArray.begin(), RandomArray.end(), 0);
        std::default_random_engine engine(seed);
        shuffle(RandomArray.begin(), RandomArray.end(), engine);
        RandomArray.insert(RandomArray.end(), RandomArray.begin(), RandomArray.end());
    }
    ...
    double PerlinNoise::Noise(double x, double y) 
    {
        ...
        int aa = RandomArray[RandomArray[X] + Y];
        int ab = RandomArray[RandomArray[X] + Y + 1];
        int ba = RandomArray[RandomArray[X + 1] + Y];
        int bb = RandomArray[RandomArray[X + 1] + Y + 1];

        double res = Lerp(v, Lerp(u, Grad(aa, x, y), Grad(ba, x - 1, y)), Lerp(u, Grad(ab, x, y - 1), Grad(bb, x - 1, y - 1)));
        return (res + 1.0) / 2.0;
    }

    std::vector<std::vector<double>> PerlinNoise::GenerateHeightMap(int width, int height)
    {
        ...
        for (int x = 0; x < width; ++x)
        {
            ...
            {
                double noiseValue = Noise(x * frequency / width, y * frequency / height, 0.0);
                heightMap[x][y] = (noiseValue + 1) / 2 * amplitude;
            }
        }
